...
3. Прочитать только текущий `docs/tasks/TASK-NNN.md`.
4. Проверить состояние файлов, статус TASK и завершённость её зависимостей.
5. Читать `ROADMAP.md`, `ARCHITECTURE.md` или `DECISIONS.md` только когда текущая
   TASK прямо зависит от launch order, архитектуры или продуктового решения.
...
- текущий `TASK-NNN.md` — результат, проверки и статус;
- `PROJECT_STATUS.md` — факт результата и только один следующий task;
- `ROADMAP.md` — отметить выполненный launch step `DONE` и ровно один следующий `NEXT`;
- `DECISIONS.md` — только для нового принятого продуктового решения;
- `CHANGELOG.md` — краткая запись.

Финальный handoff должен содержать:

```text
TASK-NNN: COMPLETED | BLOCKED
Сделано:
Проверено:
Не сделано:
Следующая задача: TASK-MMM
Команда для продолжения: Выполни TASK-MMM
Блокеры/решения:
```

После handoff агент останавливается. Если TASK имеет `IN PROGRESS`, новый чат продолжает
её с последнего записанного evidence. Следующая TASK не начинается без новой команды Owner.
