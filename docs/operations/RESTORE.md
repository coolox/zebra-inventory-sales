...
`permission denied for parameter log_min_messages` даже под `postgres`. Всё
существенное применяется до неё: `statement_timeout` = `3s` для `anon` и `8s` для
`authenticated`.

Эти timeouts **отсутствуют в миграциях** и существуют только на staging и в
`roles.sql` бэкапа. Пока это не исправлено миграцией, они не появятся в новом
production project автоматически. См. раздел «Известное расхождение».
...
Rehearsal обнаружил два объекта, которые есть на staging, но не воспроизводятся из
цепочки миграций:

1. Функция `rls_auto_enable` — присутствует в дампе staging, отсутствует во всём
   репозитории. Event trigger, который её вызывал бы, не зарегистрирован ни на
   staging (его нет в дампе), ни локально. То есть это осиротевшая функция.
2. `statement_timeout` для `anon` и `authenticated` — только в `roles.sql`.

Следствие: production project, созданный по миграциям в TASK-083, получит схему
без этих объектов. Для `statement_timeout` это ухудшение защиты, и его следует
внести миграцией до production. Для `rls_auto_enable` — решить, нужна ли функция
вообще, и либо внести её в миграции, либо удалить со staging.
