...
export function AppNav({ items, workspaceLabel, pilotStoreLabel, retailSystemLabel, storeLabel, storeMeta, profile, onNavigate, onClose, closeLabel }: Props) {
  return <>
    <div className="flex h-[72px] items-center gap-3 border-b border-zinc-800/80 px-5"><div className="flex h-9 w-9 items-center justify-center bg-violet-600 text-sm font-black tracking-tighter text-white">ZB</div><div><p className="text-sm font-bold tracking-[0.18em] text-zinc-100">ZEBRA</p><p className="text-[9px] uppercase tracking-[0.18em] text-zinc-600">{retailSystemLabel}</p></div><button type="button" onClick={onClose} className="ml-auto text-zinc-500 lg:hidden" aria-label={closeLabel}><X size={20} /></button></div>
