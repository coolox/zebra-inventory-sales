  ...
  return <section className="mt-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4" aria-label={text.title}>
    ...
    <div className="mt-4 space-y-3">
      {payments.map((payment, index) => <div key={payment.id} className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_120px_96px_36px] sm:items-end">
        ...
        <label><span className="mb-1 block text-[9px] font-semibold uppercase tracking-[0.12em] text-zinc-600">{text.amount}</span><input disabled={disabled} aria-label={`${text.amount} ${index + 1}`} type="number" min="0.01" step="0.01" value={payment.amount || ""} onChange={(event) => { if (acceptsFocusedInput(event)) update(payment.id, { amount: Number(event.target.value) }); }} onKeyDown={(event) => { if (isKeyboardTerminator(event.key) && !(event.nativeEvent as KeyboardEvent).isComposing) { event.preventDefault(); event.currentTarget.blur(); } }} placeholder="0.00" className="h-10 w-full rounded-lg border border-zinc-800 bg-zinc-950/60 px-3 text-xs text-zinc-100 outline-none focus:border-violet-500 disabled:opacity-50" /></label>
