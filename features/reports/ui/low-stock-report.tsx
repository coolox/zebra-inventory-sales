    ...
    <section className="mt-5 border-t border-zinc-800 pt-5" aria-label={text.title}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex shrink-0 gap-2">
          ...
          {isOpen ? <button type="button" onClick={() => setIsOpen(false)} className="secondary-action flex h-9 items-center gap-1.5 rounded-lg border border-zinc-700 px-3 text-xs font-semibold text-zinc-400 transition hover:border-zinc-600 hover:text-zinc-200"><ChevronDown size={14} aria-hidden="true" /> {text.hide}</button> : <button type="button" disabled={state !== "ready"} aria-expanded="false" onClick={() => setIsOpen(true)} className="secondary-action flex h-9 items-center gap-1.5 rounded-lg border border-violet-500/35 bg-violet-500/10 px-3 text-xs font-semibold text-violet-200 transition hover:bg-violet-500/15 disabled:cursor-not-allowed"><ChevronDown size={14} aria-hidden="true" /> {text.view}</button>}
        </div>
      </div>
