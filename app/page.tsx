    ...
    <DashboardShell nav={nav} mobileOpen={mobileNav} onMobileClose={() => setMobileNav(false)} mobileNavLabel={text.workspace}>
        ...
        <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:py-8 xl:px-8">
          ...
          <section id="overview" className="scroll-mt-24">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h1 className="mt-2 text-2xl font-semibold tracking-[-0.035em] sm:text-3xl">{role === "owner" ? text.businessPulse : locale === "tr" ? `Merhaba ${displayName.split(" ")[0] || "Zebra"},` : `Hello ${displayName.split(" ")[0] || "Zebra"},`}</h1>
          ...
          <section className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.75fr)]">
            <InventoryList products={products} store={roleStore} onSelect={setSelectedProductCode} labels={{ title: text.stock, search: text.search, empty: text.noResults, units: text.units, unitsShort: text.unitsShort, sku: text.sku, purchase: text.purchase, noPhoto: text.noPhoto }} actions={role === "owner" ? <><button type="button" onClick={() => { void refreshSuppliers(); setModal("suppliers"); }} className="secondary-action flex h-9 items-center justify-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-900 px-3 text-[10px] font-semibold text-zinc-300 transition hover:border-violet-500/50 hover:text-white"><Store size={13} /> {text.suppliers}</button><button type="button" onClick={() => setModal("count")} className="secondary-action flex h-9 items-center justify-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-900 px-3 text-[10px] font-semibold text-zinc-300 transition hover:border-violet-500/50 hover:text-white"><Boxes size={14} /> {text.stockCount}</button><button type="button" onClick={() => setModal("archived")} className="secondary-action col-span-2 flex h-10 items-center justify-center gap-2 rounded-lg border border-violet-500/30 bg-violet-500/10 px-3 text-xs font-semibold text-violet-200 transition hover:bg-violet-500/15 sm:col-auto sm:h-9"><ArchiveRestore size={14} /> {locale === "tr" ? "Arşivlenmiş ürünler" : "Archived products"}<span className="rounded-md bg-violet-400/15 px-1.5 py-0.5 text-[10px]">{archivedProductCount}</span></button></> : undefined} />
