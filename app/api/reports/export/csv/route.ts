...
export async function GET(request: NextRequest) {
  ...
  if (!storeId || !validDate(from) || !validDate(to) || from! > to! || !["summary", "breakdown", "inventory"].includes(report) || !dimensions.has(dimension)) return NextResponse.json({ error: "Invalid export filter" }, { status: 400 });
  ...
  const rpc = report === "summary" ? "get_reporting_metrics" : report === "breakdown" ? "get_reporting_breakdown" : "get_inventory_report";
  ...
  const rows = data ?? [];
  const headers = report === "summary" ? ["revenue_eur", "cost_eur", "margin_eur", "sale_count", "units", "average_ticket_eur"] : report === "breakdown" ? ["dimension_key", "dimension_label", "revenue_eur", "cost_eur", "margin_eur", "units"] : ["model_code", "model_name", "color", "size", "balance", "sold_units", "sell_through", "turnover", "low_stock_threshold", "is_low_stock"];
