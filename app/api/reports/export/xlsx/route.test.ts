...
describe("report XLSX API", () => {
  ...
  it("returns a typed XLSX workbook for an active Owner", async () => {
    ...
    expect(response.status).toBe(200); expect(response.headers.get("content-type")).toContain("spreadsheetml"); expect(files["xl/worksheets/sheet1.xml"]).toContain('<c r="B10" s="5"><v>100</v>'); expect(files["xl/worksheets/sheet2.xml"]).toContain("Elif"); expect(files["xl/worksheets/sheet3.xml"]).toContain("cash");
  });
});
