...
describe("report XLSX API", () => {
  ...
  it("returns a typed XLSX workbook for an active Owner", async () => {
    ...
    const response = await GET(request()); const workbook = new ExcelJS.Workbook(); await workbook.xlsx.load(await response.arrayBuffer());
    expect(response.status).toBe(200); expect(response.headers.get("content-type")).toContain("spreadsheetml"); expect(workbook.getWorksheet("Summary")?.getCell("B10").value).toBe(100); expect(workbook.getWorksheet("Breakdown")?.getCell("B6").value).toBe("Elif");
  });
});
