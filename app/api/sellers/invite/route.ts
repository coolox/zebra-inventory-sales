...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    captureServerError({ operation: "seller.invite.configuration", error, request, context: { stage: "admin_client" } });
  ...
  try {
    ...
    captureServerError({ operation: "seller.invite.auth", error, request, context: { stage: "invite_email" } });
  ...
  if (!invitedUser && /already been registered|already exists/i.test(invite.error?.message ?? "")) {
    ...
    if (error) {
      captureServerError({ operation: "seller.invite.auth", error, request, context: { stage: "existing_user_lookup" } });
      return domainError("unavailable", 502);
    }
  ...
  if (!invitedUser) {
    captureServerError({ operation: "seller.invite.auth", error: invite.error ?? new Error("Invitation returned no user"), request, context: { stage: "invite_email" } });
    return domainError("unavailable", 502);
  }
