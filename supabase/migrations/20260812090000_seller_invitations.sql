-- Zebra Retail · membership finalization after a server-side Auth invite.
...
create table public.seller_invitations (
  ...
  unique (store_id, invited_user_id)
);
...
create policy "seller invitations: owner read" on public.seller_invitations for select to authenticated using (public.user_is_store_owner(store_id));
...
$$;
revoke all on function public.activate_invited_seller(uuid, uuid, text, text, text, uuid) from public;
grant execute on function public.activate_invited_seller(uuid, uuid, text, text, text, uuid) to authenticated;
